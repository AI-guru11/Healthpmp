...
import type { UserProfile, FoodLogEntry } from '../types/health';
...
export const DashboardPage = () => {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        ...
        const { userProfiles, foodLog } = await initDatabase();

        // Load profile
        const encrypted = await userProfiles.getItem<string>(resolvedProfileId);
        if (!encrypted) throw new Error('لم يتم العثور على بيانات هذا الملف. قد يكون تم حذف التخزين المحلي.');
        ...
        // Load today's food log totals
        const start = startOfTodayMs();
        let total = 0;
        let count = 0;

        await foodLog.iterate((value: any) => {
          const entry = value as FoodLogEntry;
          if (entry?.timestamp && entry.timestamp >= start) {
            total += Number(entry.detectedCalories || 0);
            count += 1;
          }
        });
  ...
  const remaining = useMemo(() => {
    ...
    return Math.max(0, profile.tdee - todayCalories);
    ...
    <Page>
      <Card>
        <Header>
          <TitleWrap>
            <h1>لوحة التحكم</h1>
            <p>عرض ملفك الصحي المحفوظ + استهلاكك اليومي.</p>
          </TitleWrap>
        ...
        {!loading && profile && (
          <>
            ...
            <TodayBox>
              ...
              <div className="row">
                <Metric>
                  <div className="k">المستهلك اليوم</div>
                  <div className="v">{todayCalories}</div>
                  <div className="hint">سعرة/يوم (من سجلات المنتجات)</div>
                </Metric>
                <Metric>
                  <div className="k">المتبقي</div>
                  <div className="v">{remaining ?? '—'}</div>
                  <div className="hint">TDEE - المستهلك</div>
                </Metric>
                <Metric>
                  <div className="k">عدد الإضافات</div>
                  <div className="v">{todayCount}</div>
                  <div className="hint">عدد المنتجات المضافة اليوم</div>
                </Metric>
              </div>

              <Message $kind="info" style={{ marginTop: 12 }}>
                ملاحظة: السجل اليومي محفوظ على نفس الجهاز/المتصفح فقط.
              </Message>
            </TodayBox>

            <MacroBox>
              <h2>الماكروز اليومية (هدفك اليومي)</h2>
              <div className="row">
                <div className="item">
